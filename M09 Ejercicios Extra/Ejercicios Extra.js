...
function numberOfCharacters(string) {
   ...
   for(var i=0;i<string.length;i++){
      if(objetoNew.hasOwnProperty(string[i])){
         objetoNew[string[i]]=objetoNew[string[i]]+1;
      }
      else{
         objetoNew[string[i]]=1;
      }
   }
   return objetoNew;
}
...
function capToFront(string) {
   ...
   for(var i=0;i<string.length;i++){
      ...
      else{mayúsculas=mayúsculas+string[i]}
   }
...
function asAmirror(frase) {
   ...
   var arrayFinal= toArray.map(function(ele){
...
function capicua(numero) {
   ...
   if(nroString===nroString.split('').reverse().join('')){
      return 'Es capicua';
   }
